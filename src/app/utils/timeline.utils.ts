...
import { addDays, addHours, diffInDays, formatDayLabel, formatHourLabel, formatMonthLabel, formatWeekLabel, startOfDay } from './date.utils';
...
function buildMonthColumns(rangeStart: Date, rangeEnd: Date): TimelineColumn[] {
  ...
  while (s < end) {
    ...
    cols.push({ start: new Date(s), end: e, label: formatMonthLabel(s) });
    s = e;
  }
  return cols;
}
...
function buildDayColumns(rangeStart: Date, daysCount: number): TimelineColumn[] {
  ...
  for (let i = 0; i < daysCount; i++) {
    ...
    cols.push({ start: s, end: e, label: formatDayLabel(s) });
  }
  return cols;
}
...
function buildWeekColumns(rangeStart: Date, weeksCount: number): TimelineColumn[] {
  ...
  for (let i = 0; i < weeksCount; i++) {
    ...
    cols.push({ start: s, end: e, label: formatWeekLabel(s, addDays(e, -1)) });
  }
  return cols;
}
...
function buildHourColumns(rangeStart: Date, hoursCount: number): TimelineColumn[] {
  ...
  for (let i = 0; i < hoursCount; i++) {
    ...
    cols.push({ start: s, end: e, label: formatHourLabel(s, true) });
  }
  return cols;
}
...
/** Initial range for infinite scroll. */
export function getInitialRange(timescale: Timescale, today: Date): TimelineRange {
  ...
  if (timescale === 'week') {
    return { rangeStart: addDays(t, -56), rangeEnd: addDays(t, 56) };
  }
  if (timescale === 'day') {
    return { rangeStart: addDays(t, -14), rangeEnd: addDays(t, 14) };
  }
  const rangeStart = new Date(t.getFullYear(), t.getMonth(), t.getDate(), 0, 0, 0);
  return { rangeStart, rangeEnd: addHours(rangeStart, 48) };
}

/** Extend range to the left (past). Returns new rangeStart and the pixel width added.
 * @upgrade Consider configurable extend amounts per timescale (e.g. 2 months vs 4). */
export function extendRangeLeft(
  range: TimelineRange,
  timescale: Timescale,
  amount: number
): { newRangeStart: Date; addedWidthPx: number } {
  const pxPerDay = timescale === 'month' ? 18 : timescale === 'week' ? 24 : timescale === 'day' ? 56 : 60;
  const pxPerHour = 56;
  if (timescale === 'month') {
    const newStart = new Date(range.rangeStart.getFullYear(), range.rangeStart.getMonth() - amount, 1);
    const colWidth = 110;
    return { newRangeStart: newStart, addedWidthPx: amount * colWidth };
  }
  if (timescale === 'week') {
    const newStart = addDays(range.rangeStart, -amount * 7);
    return { newRangeStart: newStart, addedWidthPx: amount * 7 * pxPerDay };
  }
  if (timescale === 'day') {
    const newStart = addDays(range.rangeStart, -amount);
    return { newRangeStart: newStart, addedWidthPx: amount * pxPerDay };
  }
  const newStart = addHours(range.rangeStart, -amount);
  return { newRangeStart: newStart, addedWidthPx: amount * pxPerHour };
}

/** Extend range to the right (future). Returns new rangeEnd. */
...
export function buildTimelineFromRange(timescale: Timescale, range: TimelineRange): TimelineModel {
  if (timescale === 'month') {
    ...
    return { timescale, rangeStart: range.rangeStart, rangeEnd, columns, pxPerDay: 18 };
  }
  if (timescale === 'week') {
    ...
    const rangeEnd = columns.length > 0 ? columns[columns.length - 1].end : range.rangeEnd;
    return { timescale, rangeStart: range.rangeStart, rangeEnd, columns, pxPerDay: 24 };
  }
  if (timescale === 'day') {
    const days = diffInDays(range.rangeEnd, range.rangeStart);
    const columns = buildDayColumns(range.rangeStart, days);
    return { timescale, rangeStart: range.rangeStart, rangeEnd: range.rangeEnd, columns, pxPerDay: 56 };
  }
  ...
  const rangeEnd = columns.length > 0 ? columns[columns.length - 1].end : range.rangeEnd;
  return { timescale, rangeStart: range.rangeStart, rangeEnd, columns, pxPerDay: 60, pxPerHour: 56 };
}

/** @deprecated Use getInitialRange + buildTimelineFromRange for infinite scroll. */
